/**
 * Produced by the client build (which runs after the server build) and read back at
 * runtime. Chunk keys are basenames without `.js` (e.g. `Counter.abc123`) — the same
 * form the islands emitter derives from an island's public chunk URLs.
 *
