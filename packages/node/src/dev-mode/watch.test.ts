...
function createMockServer(opts?: {
  bootDeps?: string[] | undefined;
  extraModules?: { file: string; deps?: string[] | undefined }[] | undefined;
}) {
  ...
  const createMod = (file: string, deps: string[]) => ({
    file,
  ...
  const modules = new Map([
    [bootFile, createMod(bootFile, opts?.bootDeps ?? [])],
    ...(opts?.extraModules ?? []).map((mod) => [mod.file, createMod(mod.file, mod.deps ?? [])] as const),
  ]);
...
describe('setupBootWatch', () => {
  ...
  describe('multiple watch entries', () => {
    test('schedules when a second entry file itself changes', () => {
      const server = createMockServer({ extraModules: [{ file: '/project/src/config.ts' }] });
      const scheduler = createMockScheduler();

      setupBootWatch(server as never, ['src/boot.ts', 'src/config.ts'], scheduler as never);

      server.watcher.emit('change', '/project/src/config.ts');

      expect(scheduler.schedule).toHaveBeenCalledTimes(1);
      expect(scheduler.schedule).toHaveBeenCalledWith(server, '/project/src/config.ts');
    });

    test('schedules when a transitive dependency of a second entry changes', () => {
      const server = createMockServer({
        extraModules: [{ file: '/project/src/config.ts', deps: ['/project/src/schema.ts'] }],
      });
      const scheduler = createMockScheduler();

      setupBootWatch(server as never, ['src/boot.ts', 'src/config.ts'], scheduler as never);

      server.watcher.emit('change', '/project/src/schema.ts');

      expect(scheduler.schedule).toHaveBeenCalledTimes(1);
      expect(scheduler.schedule).toHaveBeenCalledWith(server, '/project/src/schema.ts');
    });

    test('boot dependencies keep scheduling alongside extra entries', () => {
      const server = createMockServer({
        bootDeps: ['/project/src/services.ts'],
        extraModules: [{ file: '/project/src/config.ts' }],
      });
      const scheduler = createMockScheduler();
    ...
    test('an entry absent from the module graph is inert', () => {
      const server = createMockServer();
      const scheduler = createMockScheduler();

      setupBootWatch(server as never, ['src/boot.ts', 'src/not-imported.ts'], scheduler as never);

      server.watcher.emit('change', '/project/src/not-imported.ts');
