...
describe('recordStaleError', () => {
  test('flushes a single aggregated log line per burst', async () => {
    ...
    expect(log.mock.calls[0]![0]).toBe(
      '[@astroscope/boot] suppressed 3 stale request errors from a previous app generation',
    );
  });
  ...
  test('uses singular phrasing for a single error', async () => {
    ...
    expect(log.mock.calls[0]![0]).toBe(
      '[@astroscope/boot] suppressed 1 stale request error from a previous app generation',
    );
  });
